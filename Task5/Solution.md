...
2. **Назначьте метки для сервисов** Метки выполняют функцию ролей для сервиса: 1. `front-end` 2. `back-end-api` 3. `admin-front-end` 4. `admin-back-end-api`
...
Чтобы работал NetworkPolicy нужно стартовать миникуб так
```bash
minikube delete
minikube start --driver=docker --cni=calico
```

Дефолтный cni не фильтрует. 

Для воспроизведения необходимо запустить следующие команды
...
# внутри пода:
wget -qO- --timeout=2 http://back-end-api-app         # должно ответить (HTML nginx)
wget -qO- --timeout=2 http://admin-back-end-api-app   # должен быть timeout

kubectl run test-$RANDOM --rm -i -t --image=alpine --labels role=admin-front-end -- sh
wget -qO- --timeout=2 http://admin-back-end-api-app   # ответит
wget -qO- --timeout=2 http://back-end-api-app         # timeout
```